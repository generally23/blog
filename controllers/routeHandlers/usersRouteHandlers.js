...
const { validate, generateToken } = require("../../utils");
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!email) {
    return next(
      new ApplicationError(
        "You must provide you email to reset your password",
        400
      )
    );
  }
  ...
  try {
    ...
    res.json(
      new ApplicationResponse({
        message:
          "We have successgully sent you an email to reset your password",
      })
    );
  } catch (e) {
    console.log(e);
